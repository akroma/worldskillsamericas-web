...
<div class="xcrud">
    ...
    <h2 class="xcrud-header">
        <a class="xcrud-toggle<?php echo ($this->start_minimized) ? ' closed' : '' ?> pull-right btn" href="javascript:void(0);"><i class="icon-resize-vertical"></i></a>
        <?php echo $this->table_name ?>
        <?php echo $this->get_table_tooltip('bs'); ?>
    </h2>
    <?php } ?>
...
<?php } ?>
    ...
    <?php } ?>
        ...
        <div class="xcrud-top-actions">
            <?php if ($this->is_csv)
        { ?>
                <button type="submit" class="xcrud-action-button xcrud-csv pull-right btn"><i class="icon-file"></i> <?php echo $this->lang('export_csv')?></button>
            <?php }
        if ($this->is_print)
        { ?>
                <a class="xcrud-print xcrud-action-button btn pull-right" href="javascript:void(0);" onclick="jQuery.print_window(window.open('', '<?php echo $this->lang('print')?>','height=460,width=760'),jQuery(this).closest('.xcrud-ajax'));"><i class="icon-print"></i> <?php echo $this->lang('print')?></a>
            <?php }
        if ($this->is_add)
        { ?>
                <a class="xcrud-add xcrud-action-button btn btn-success" href="javascript:void(0);"><i class="icon-plus-sign icon-white"></i> <?php echo $this->lang('add')?></a>
        <?php }
